...
class ColorBar(QWidget):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            self.emit(SIGNAL("colorSelected"), color)
            event.accept()
...
class Plot(Qwt.QwtPlot):
    ...
    def __init__(self, *args):
        ...
        scaleDraw.setMinimumExtent(scaleDraw.extent(
            QPen(), self.axisWidget(Qwt.QwtPlot.yLeft).font()))
        ...
        self.__wheel.setOrientation(Qt.Vertical)
...
class ScalePicker(QObject):
    ...
    def __mouseClicked(self, scale, pos):
        ...
        self.emit(SIGNAL("clicked"), axis, value)
